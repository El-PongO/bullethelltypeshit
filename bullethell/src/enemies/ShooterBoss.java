...
public class ShooterBoss extends Enemy {
    ...
    public BufferedImage[] walkSprites = new BufferedImage[12]; // 12 walk sprites (0-11)
    public BufferedImage[] idleSprites = new BufferedImage[6]; // 6 idle sprites 
    public BufferedImage[] jumpSprites = new BufferedImage[8]; // 8 jump sprites (0-7)
    ...
    public ShooterBoss(int x, int y) {
        super(x, y);        
        // Shooter Boss properties        this.shootDelay = 3000; // 3 seconds between burst sequences
        this.bulletSpeed = 4;   // Faster bullets than regular enemies
        this.health = 750;      // Increased health from 400 to 750
        this.maxHealth = 750;
        this.size = 43;         // Reduced by 3 times from 130 to 43
    ...
    public void getEnemyImage() {
        try {
            // Load walk sprites (12 frames, 0-11)
            for (int i = 0; i < 12; i++) {
            ...
            // Load jump sprites (8 frames, 0-7) from ChargeJump000.png to ChargeJump007.png
            try {
                for (int i = 0; i < 8; i++) {
                ...
                System.out.println("Error loading ChargeJump sprites: " + e.getMessage());
        ...
        // Burst shooting behavior
            ...
            if (burstTimer <= 0) {
                ...
                if (burstCounter >= shotsPerBurst) {
                    ...
                    // Only jump after completing all shots
    ...
    private void updateSpriteCounter() {
        ...
        if (this.spritecounter > animSpeed) {
            if (isJumping) {
                ...
                // Keep the frame within range 0-7
                jumpFrame = (jumpFrame + 1) % 8;
            } else {
                // Advance walk or idle animation
                // Use walkSprites.length (12) or idleSprites.length (6) based on state
    ...
    public void draw(Graphics g, int ex, int ey) {
        ...
        if (isJumping) {
          ...
          // Draw with appropriate size while maintaining aspect ratio
        if (bimage != null) {
